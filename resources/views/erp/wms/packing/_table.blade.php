<div class="table-responsive">
    <table class="table table-vcenter table-mobile-md card-table">
        ...
        <tbody>
            ...
            <tr>
                <td><input type="checkbox" class="form-check-input order-checkbox" name="ids[]" value="{{ $order->id }}"></td>
                <td data-label="Order Details">
                    <div class="d-flex py-1 align-items-center">
                        <div class="flex-fill">
                            <div class="font-weight-medium text-primary">{{ $order->order_number }}</div>
                            <div class="text-secondary small">{{ $order->party->name ?? 'N/A' }}</div>
                        </div>
                    </div>
                </td>
                <td data-label="Warehouse">
                    <div class="badge bg-outline text-muted">{{ $order->warehouse->name ?? '-' }}</div>
                </td>
                <td class="text-center" data-label="Complexity">
                    <span class="badge badge-pill bg-blue-lt">{{ $order->items_count ?? $order->items->count() }} SKU</span>
                    <div class="small text-secondary mt-1">{{ $order->packages->count() }} pkg(s)</div>
                </td>
                <td data-label="Fulfillment">
                    ...
                    <div class="small text-muted">{{ $packedCount }}/{{ $totalPkg }} sealed</div>
                </td>
                <td data-label="Status">
                    ...
                    @endphp
                    <span class="badge {{ $badgeCls }} text-uppercase">{{ str_replace('_',' ',$order->status) }}</span>
                </td>
                ...
                <td>
                    <div class="btn-list flex-nowrap">
                        ...
                        <form method="POST" action="{{ route('erp.wms.packing.start', $order) }}">
                            @csrf
                            <button class="btn btn-warning btn-sm">Start Packing</button>
                        </form>
                        @else
            ...
            <tr>
                <td colspan="8">
                    <div class="empty">
                        <div class="empty-img"><img src="{{ asset('tabler/static/illustrations/light/archive.png') }}" height="128" alt=""></div>
...
<div class="card-footer d-flex align-items-center border-top-0" id="pagination-links">
    <p class="m-0 text-secondary d-none d-sm-block">Showing <span>{{ $orders->firstItem() }}</span> to <span>{{ $orders->lastItem() }}</span> of <span>{{ $orders->total() }}</span> entries</p>
    <div class="ms-auto">
