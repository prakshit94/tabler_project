    ...
    <table class="table table-vcenter table-mobile-md card-table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td data-label="Order Details">
                    <div class="d-flex py-1 align-items-center">
                        <div class="flex-fill">
                            <div class="font-weight-medium">{{ $pl->order->party->name ?? 'N/A' }}</div>
                            <div class="text-secondary">
                                <a href="{{ route('erp.orders.show', $pl->order_id) }}" class="text-reset">Order: {{ $pl->order->order_number ?? '-' }}</a>
                            </div>
                        </div>
                    </div>
                </td>
                <td data-label="Warehouse">
                    <div class="badge bg-outline text-muted">{{ $pl->warehouse->name ?? '-' }}</div>
                </td>
                <td data-label="Assignment">
                    @if($pl->assignedTo)
                        <div class="d-flex align-items-center">
                            <span class="avatar avatar-xs me-2 rounded text-uppercase bg-blue-lt">{{ substr($pl->assignedTo->name, 0, 1) }}</span>
                            {{ $pl->assignedTo->name }}
                        </div>
                    @else
                ...
                <td class="text-center" data-label="Items">
                    <span class="badge badge-pill bg-blue-lt">{{ $pl->items_count ?? $pl->items()->count() }} SKU</span>
                </td>
                <td data-label="Status">
                    ...
                    @endphp
                    <span class="badge {{ $badgeCls }} text-uppercase">{{ str_replace('_',' ',$pl->status) }}</span>
                </td>
            ...
            <tr>
                <td colspan="9">
                    <div class="empty">
                        <div class="empty-img"><img src="{{ asset('tabler/static/illustrations/light/printer.png') }}" height="128" alt=""></div>
...
<div class="card-footer d-flex align-items-center border-top-0" id="pagination-links">
    <p class="m-0 text-secondary d-none d-sm-block">Showing <span>{{ $pickLists->firstItem() }}</span> to <span>{{ $pickLists->lastItem() }}</span> of <span>{{ $pickLists->total() }}</span> entries</p>
    <div class="ms-auto">
